...
Actions are triggers of state changes, they get dispatched to the storefor processing. The reducers define how based on these action the state has to be changed. The store itself stores the sate and his reducers.
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
Componenet state is strictly for the componentn, and would get lost on rerendering, while application state is more consistent and doesnt depend on the life of components. Therefore iit is useful to store data which is integral for continuious functionality, while component state is very useful for temporal state.
